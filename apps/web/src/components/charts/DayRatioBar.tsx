...
export const DayRatioBar = ({ logs }: { logs: Log[] }) => {
  ...
  if (isEmpty || totalDuration <= 0) {
    return (
      <div className="flex h-1.5 w-full min-w-0 overflow-hidden bg-[#282e29]" />
    );
  }
  ...
  return (
    <div
      className="isolate flex h-1.5 w-full min-w-0 transform-gpu flex-nowrap overflow-hidden bg-[#282e29]"
      ...
      {blocks.map((block, idx) => (
        <div
          ...
          className={`transition-all duration-500 ease-in-out ${
            block.type === 'productive' ? 'bg-[#c9ff3d]' : 'bg-[#ffb52e]'
          }`}
